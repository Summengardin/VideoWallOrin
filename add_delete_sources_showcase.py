...
def add_filesink(pipeline):
    # nvv4l2h264enc -> h264parse -> mp4mux -> filesink location=test.mp4'
    
    enc = Gst.ElementFactory.make("nvv4l2h264enc", "nvenc")
    ...
    # enc = Gst.ElementFactory.make("nvvidconv", "nvenc")
    # sink = Gst.ElementFactory.make("xvimagesink", "mux")
...
def bus_call(bus, message, loop):
    ...
    if t == Gst.MessageType.EOS:
        ...
        sys.stderr.write("Bus message: Warning: %s: %s\n" % (err, debug))
    elif t == Gst.MessageType.ERROR:
        err, debug = message.parse_error()
        sys.stderr.write("Bus message: Error: %s: %s\n" % (err, debug))
        # loop.quit()
...
def increment_sources():
    ...
    print(f"Current state: {g_inc_state}")

    if g_inc_dir > 0 and g_inc_state == 4:
        g_inc_dir *= -1
    if g_inc_dir < 0 and g_inc_state == 1:
        g_inc_dir *= -1
    ...
    try:
        ...
        if g_inc_dir < 0:
            if g_inc_state == 4:
                ...
                stop_release_source(source_id=1)
                g_inc_state -= 1
            elif g_inc_state == 2:
